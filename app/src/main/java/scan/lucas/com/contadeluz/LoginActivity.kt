...
class LoginActivity : AppCompatActivity(), LoaderCallbacks<Cursor> {
    ...
    private fun attemptLogin() {
        ...
        if (cancel) {
            ...
            var user = Usuario()
            user.email = emailStr
            user.senha = passwordStr
            val request = controllerApi.AutenticaUsuario(user)
            request.enqueue(object : Callback<Usuario> {
                override fun onResponse(call: Call<Usuario>, response: Response<Usuario>) {
                    showProgress(false)
                    if (response != null && response.isSuccessful) {

                        val _user: Usuario? = response.body() as? Usuario
                        ...
                        //Toast.makeText(this@LoginActivity, "Nenhum ", Toast.LENGTH_LONG).show()
                    }
                }

                override fun onFailure(call: Call<Usuario>, t: Throwable) {
                    showProgress(false)
                    Toast.makeText(this@LoginActivity, "Erro ao realizar o login: " + t.message, Toast.LENGTH_LONG).show()
                }
            })
        }
    }
